...
CREATE TABLE `auditoria` (
  ...
  `CodReg` int DEFAULT NULL,
  `CodOperacion` char(1) DEFAULT NULL,
...
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `AuditoriaAltaCliente` AFTER INSERT ON `cliente` FOR EACH ROW INSERT INTO Auditoria
(Usuario, Fecha, CodReg, CodOperacion)
VALUES
(new.ClienteNom,now(),new.ClienteID,'A') */;;
DELIMITER ;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `AuditoriaModCliente` AFTER UPDATE ON `cliente` FOR EACH ROW BEGIN
DECLARE NomCli VARCHAR(50);
DECLARE DniCli VARCHAR(50);
DECLARE CorreoCli VARCHAR(50);
DECLARE AuditoriaDesc VARCHAR(100);
SET @NomCli = ' ';
SET @DniCli = ' ';
SET @CorreoCli = ' ';
IF new.ClienteNom <> old.ClienteNom then
SET @NomCli = 'NOMBRE ';
   SET @AuditoriaDesc = concat(@NomCli);
    END IF;
IF new.ClienteDNI <> old.ClienteDNI then
SET @DniCli = ' DNI';
    SET @AuditoriaDesc = concat(@NomCli,  @DniCli);
    END IF;
 IF new.ClienteCorreo <> old.ClienteCorreo then
SET @CorreoCli = ' CORREO';
    SET @AuditoriaDesc = concat(@NomCli,  @DniCli, @CorreoCli);
     END IF;
INSERT INTO Auditoria
(Usuario ,Fecha, CodReg, CodOperacion, Descripcion) 
VALUES 
(new.ClienteNom, now(), old.ClienteID,'M',@AuditoriaDesc);
END */;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
CREATE TABLE `pedido` (
  ...
  `ProdID` int NOT NULL,
  `PedidoFecha` timestamp NULL DEFAULT NULL,
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `AuditoriaProducto` AFTER UPDATE ON `producto` FOR EACH ROW BEGIN   
DECLARE PuntoReposicion INT;
SET @PuntoReposicion = 10;

IF new.ProdStock < @PuntoReposicion  then
INSERT INTO pedido
(ProdID,PedidoFecha)
VALUES
(new.ProdID, now());
...
-- Dump completed on 2022-05-04  0:57:22
